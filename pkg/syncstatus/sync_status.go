...
// CloudKit zone names are part of the persisted schema and intentionally live
// here rather than depending on the connector package. This package is used by
// the CGO-free CLI as well as the running connector command.
const (
...
type ZoneSyncStatus struct {
	...
	// HasError reports that CloudKit recorded a failed attempt. The persisted
	// error text is intentionally never retained or rendered: it can contain
	// Apple handles, URLs, record identifiers, and transport credentials.
	HasError bool
	// LastError is retained as a deprecated compatibility field for callers
	// that constructed reports themselves. Format never renders its contents.
	LastError string
	UpdatedAt *time.Time
}
...
type SyncStatusReport struct {
	...
	// UnavailableMessages are rows that cannot be delivered from the local
	// persisted state: a scrubbed row with no matching bridge message, or an
	// orphan with neither a live cloud_chat row nor a CloudKit chat identifier.
	// They are excluded from the pending denominator rather than reported as a
	// forever-stuck delivery.
...
func syncStatusTableExists(ctx context.Context, db *dbutil.Database, table string) (bool, error) {
	...
	case dbutil.Postgres:
		err = db.QueryRow(ctx,
			`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=current_schema() AND table_name=$1`, table).Scan(&count)
	default:
		err = db.QueryRow(ctx,
...
// of one cloud_message row. A portal can contain multiple live cloud_chat
// siblings with different filtered state, so portalBridgeableSQL alone would
...
// sqlInstrFunc returns the dialect's one-based substring-position function.
// SQLite calls it instr; PostgreSQL calls it strpos. Keeping this small
// dialect switch in the CGO-free package is what lets both the CLI and the
// running connector use the same delivered-GUID query on either backend.
...
// readZones fills in per-zone last success and whether the last attempt failed.
// A zone with no row has never been synced at all, which is a different (and
// more useful) statement than "synced, no error". The persisted error text is
// scanned only to derive HasError and is never copied into the report.
...
// delivery predicate without importing connector. A scrubbed row is contentful
// only when its bridgev2 message row is present; otherwise it is an orphaned
// privacy tombstone and must not remain pending forever.
...
//     a body_scrubbed=TRUE row counts as content only when its matching
//     bridgev2 message row exists. A scrubbed row without delivery evidence is
//     classified as unavailable, not left pending forever. has_body=FALSE,
//     group-renames, empty bodies, and senderless system records stay out of
//     the deliverable set; senderless direct-message rows retain the same
//     portal-ID fallback as makeCloudSender, while group/system rows do not.
...
//   - The cap window is the newest N bridgeable source rows per portal by
...
//     slot because the store excludes them before LIMIT. Ranking still happens
//     before the contentful filter because readable reaction/system rows do
//     consume slots in the store.
