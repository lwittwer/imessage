...
# after a manually interrupted setup). Setup never deletes it implicitly:
# registration deletion may remove remote Matrix rooms and requires reset confirmation.
...
if [ -n "${DISABLE_FACETIME:-}" ]; then
    ...
    echo "  The bridge stays registered either way; this setting only controls notices."
    echo ""
...
echo "  Config: $CONFIG"
echo ""
if { [ "${SYSTEMD_MODE:-none}" = "user" ] && [ -f "${USER_SERVICE_FILE:-}" ]; } \
   || { [ "${SYSTEMD_MODE:-none}" = "system" ] && [ -f "${SYSTEM_SERVICE_FILE:-}" ]; }; then
    # Use the corten-matrix CLI rather than raw systemctl/journalctl — it
    # resolves user-unit vs system-unit and sudo on its own, which a single
    # printed spelling cannot.
