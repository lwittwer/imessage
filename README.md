...
> **Download the extractor:** grab it from the [`tools-v1` release](https://github.com/lwittwer/imessage/releases/tag/tools-v1). Use the tool from the binary releases; extractors from before the binary switch are not compatible.

**Option A — GUI app (recommended).** Download the hardware-key [extractor app](https://github.com/lwittwer/imessage/releases/download/tools-v1/ExtractKey.app.zip), copy it to the Mac, and launch it. It reads the hardware identifiers, displays them, and lets you **Copy** or **Save** the base64 key.
...
**Option B — CLI (fallback).** If you'd rather not use the GUI, download the [command-line extractor](https://github.com/lwittwer/imessage/releases/download/tools-v1/extract-key-cli.zip), run it on the Mac, and copy the base64 key it prints:
