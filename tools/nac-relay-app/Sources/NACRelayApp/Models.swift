...
/// Matches rustpush/src/macos.rs MacOSConfig, with optional relay fields.
struct MacOSConfig: Codable {
    ...
    var nacRelayURL: String?
    var relayToken: String?
    var relayCertFP: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case nacRelayURL = "nac_relay_url"
        case relayToken = "relay_token"
        case relayCertFP = "relay_cert_fp"
    }
}

// MARK: - RelayInfo

/// Matches the relay-info.json written by the Go nac-relay.
...
extension MacOSConfig {
    /// Serialize to JSON with key order matching the Go extract-key tool output.
    /// Go's encoding/json marshals struct fields in declaration order.
    /// Relay fields are omitted when nil (matching Go's omitempty).
    func toOrderedJSON() -> String {
        ...
        if let url = nacRelayURL, !url.isEmpty {
            pairs.append(("nac_relay_url", "\"\(jsonEscape(url))\""))
        }
        if let token = relayToken, !token.isEmpty {
            pairs.append(("relay_token", "\"\(jsonEscape(token))\""))
        }
        if let fp = relayCertFP, !fp.isEmpty {
            pairs.append(("relay_cert_fp", "\"\(jsonEscape(fp))\""))
        }
        let body = pairs.map { "\"\($0.0)\":\($0.1)" }.joined(separator: ",")
        return "{\(body)}"
    }
